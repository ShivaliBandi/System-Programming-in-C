...
void *ascending()
{
    ...
    int icount=0;
    printf("inside ascending thread\nthread id->%d\n",(int)pthread_self());
    for(int icnt=1;icnt<=500;icnt++,icount++)
    {   
        if(icount==23)
        {
            printf("\n");
...
void *descending()
{
    //pthread_self is used to get current thread's id
    int icount=0;
    printf("\ninside descending thread\nthread id->%d\n",(int)pthread_self());
    for(int icnt=500;icnt>=1;icnt--,icount++)
    {   
        if(icount==23)
        {
            printf("\n");
...
int main()
{
    ...
    */
    ...
    pthread_create(&thread1,NULL,ascending,NULL);
    pthread_join(thread1,NULL);
        //printf("in main function before creating thread\n");
    pthread_create(&thread2,NULL,descending,NULL);
    /*
